...
class BenchmarkSuiteRunner:
    class PlottingConfig:
        def __init__(self, generate_graphs: bool, generate_summary_graph: bool, generate_pdf: bool):
    ...
    def __init__(
        ...
        logger.debug(
            f"Initialized benchmark suite runner with output_dir={self.output_dir}, backends={self.backends}, params={self.params}, io_config={self.io_config}, rpc_config={self.rpc_config}, scylla_config={self.scylla_config}, benchmarks={self.benchmarks}, plotting_config={self.plotting_config}"
        )
...
def run_benchmark_suite_args(args, metadata: Metadata):
    ...
    for config_path in config_paths:
        ...
        match get_config_version(config):
            case 1:
                ...
                config = upgrade_version1_to_version2(
                    config, make_proportional_splitter(int(args.legacy_cores_per_worker))
                )
            case 2:
                pass
            case other:
        ...
        runner = BenchmarkSuiteRunner(metadata, plotting_config, safe_load(benchmark_yaml), config)
...
def configure_run_benchmark_suite_parser(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--legacy-cores-per-worker", help="used to calculate async worker cpuset when using a version 1 config"
    )
    parser.add_argument(
        "--generate-summary-graphs", help="generate summary graphs for each benchmark", action="store_true"
    )
