...
class RedrawPdfRunner:
    ...
    def run_redraw_pdf(self, dir: Path) -> None:
        ...
        for benchmark_dir in sorted(dir.iterdir()):
            ...
            benchmark_name = self._schedule_summary_pngs(summary_file, benchmark_dir)
            benchmark_dirs_to_render.append((benchmark_name, benchmark_dir))

        if benchmark_dirs_to_render:
            logger.info("Generating PNG summary graphs")
    ...
    def _schedule_summary_pngs(self, summary_file: Path, output_dir: Path) -> str:
        logger.info(f"Redrawing PDF data from {summary_file}")
...
def run_redraw_pdf_args(args: argparse.Namespace, metadata_holder: BenchmarkMetadataHolder) -> None:
    runner = RedrawPdfRunner(metadata_holder)
    runner.run_redraw_pdf(Path(args.dir))


def configure_redraw_pdf_parser(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--dir", help="directory with existing benchmark results", required=True)
    parser.set_defaults(func=run_redraw_pdf_args)
