...
from metadata import Metadata, MetricMetadata, MetricPlotMetadata, _asterix_compare
...
class TestMetadata:
    ...
    def test_repr(self) -> None:
        ...
        obj = Metadata(sharded_metrics=sharded)
        expected = f"Metadata(sharded_metrics={sharded}, shardless_metrics={TreeDict[MetricMetadata]()})"
        assert repr(obj) == expected

    def test_get_sharded_metric_metadata_or_default_present(self, sample_tree_dict) -> None:
        obj = Metadata(sharded_metrics=sample_tree_dict)
    ...
    def test_get_shardless_metric_metadata_or_default_present(self, sample_tree_dict) -> None:
        obj = Metadata(shardless_metrics=sample_tree_dict)
